...
class DynamicEnv():
    def __init__(self, opt, Predict_Scale_param):
        ...
        Grouped_GOS = self.Generated_Obstacle_Segments.reshape(self.O,4,2,2)
        ...
        self.Obs_X_limit = torch.tensor([0.13,0.82], device=self.dvc)*self.window_size # [47,300] for 366
    ...
    def Map_Init(self):
        ...
        self.x_target, self.y_target = 0.90*self.window_size, 0.90*self.window_size # 终点坐标, (330,330) for 366
        if self.Playmode:
            self.y_start = np.random.randint(0.05*self.window_size,0.90*self.window_size) # 随机起点, (18,330) for 366
            self.y_target = np.random.randint(0.05*self.window_size, 0.90*self.window_size) # 随机终点, (18,330) for 366
        ...
        self.Flat_pdct_segments = self.Grouped_pdct_segments.reshape(self.dynamic_obs * 4, 2, 2) # 用于计算OkayPlan计算交叉点个数
    ...
    def Update(self, path, act=None):
        ...
        if act is not None: # 根据键盘控制移动目标点
            ...
            self.x_target = np.clip(self.x_target, 0.88*self.window_size, self.window_size) # 注意0.88需要大于self.Obs_X_limit[1]的系数(0.82)
